...
Function{
	...
	For i In {1:Num_Super} // Full Bulk: 2.2977 Stacked Tapes : 1.5312*1e8 (Adjusted after Bobine Zoubir) (2.2977*0.3962*1.9083*0.8814) ||| Cutted Sample: 3.1786*1e8
		...
		n_Base_5 = 20;
		n_Base_6 = 20;
		DefineConstant[ jc~{i} = {jc_Base~{i}, Name Sprintf("Input/3Material Properties/2jc Sample%g (Am⁻²)",i), Visible 1}]; // Critical current density [A/m2] QUALITATIVE BULKS : 2.2977*1e8, STACK TAPES : 1.3037*1e8
    ...
    directionApplied[] = Vector[0., 1., 0.]; // y --> central ech, z --> peripheral ech
	...
	If(Active_approach == 1)
		Velocity[Cuboid_Superconductor_1] = Vector[0, 0, Approach_speed]; // Useful if we do not convect fields and add the term v/\B
		// Velocity[Cuboid_Superconductor_2] = Vector[0, 0, 0];
		// Velocity[Cuboid_Superconductor_3] = Vector[0, 0, - Approach_speed];
		// Velocity[OmegaCC] = Vector[0, 0, 0];
	Else
	...
	If(Time_step==1 && Cycle==1) // First step		
		...
		If(Num_Super == 3 || Num_Super == 4)
			...
			If(formulation == a_formulation)
				...
				// a_fromFile[Cuboid_Superconductor_2] = Vector[0, 0, 0];					// To see only the impact of the assembly process in the central bulk
	...
	Else	// All other steps
		If(Num_Super == 3 || Num_Super == 4)	
			...
			// MVT h formulation, !!!!!!!!!! give correct File !!!!!!!!!!
			...
			ElseIf(formulation == h_formulation)
				h_fromFile[] = VectorField[XYZ[]]{42}; // Ici on ne convecte pas le champ mais on rajoute le terme v/\B dans la formulation, mais mauvais à cause de projection entre maillage.
			ElseIf(formulation == coupled_formulation)
		...
		Else
			...
			// a_fromFile[Surface_Cuboid_Superconductor_1] =   Vector[0., 0. , 0.]; 
			// h_fromFile[Cuboid_Superconductor_1] =   Vector[0., 0 , 0]; 
		EndIf	
	EndIf
}
    ...
    { Name MagDyn; LastTimeStepOnly realTimeSolution ;
        ...
        Operation {
			...
			If(Save_later == 1) // Exports for MATLAB plot
				...
				If(Num_Super == 3 || Num_Super == 4)
					// Visualisation of the current density distribution inside the bulks
					// Plane OXZ
					Print[ jx, OnPlane { {-Air_Lx/2,0,-Air_Lz/2} {-Air_Lx/2,0,Air_Lz/2} {Air_Lx/2,0,-Air_Lz/2} } {1000, 1000}, File StrCat["res/For_Matlab/jx_planeOxz",Str_step,".pos"], Format Gmsh, OverrideTimeStepValue Time_step, LastTimeStepOnly, SendToServer "No"];
					Print[ jy, OnPlane { {-Air_Lx/2,0,-Air_Lz/2} {-Air_Lx/2,0,Air_Lz/2} {Air_Lx/2,0,-Air_Lz/2} } {1000, 1000}, File StrCat["res/For_Matlab/jy_planeOxz",Str_step,".pos"], Format Gmsh, OverrideTimeStepValue Time_step, LastTimeStepOnly, SendToServer "No"];
					Print[ jz, OnPlane { {-Air_Lx/2,0,-Air_Lz/2} {-Air_Lx/2,0,Air_Lz/2} {Air_Lx/2,0,-Air_Lz/2} } {1000, 1000}, File StrCat["res/For_Matlab/jz_planeOxz",Str_step,".pos"], Format Gmsh, OverrideTimeStepValue Time_step, LastTimeStepOnly, SendToServer "No"];								
					
					// Plane OYZ
					Print[ jx, OnPlane { {0,-Air_Ly/2,-Air_Lz/2} {0,-Air_Ly/2,Air_Lz/2} {0,Air_Ly/2,-Air_Lz/2} } {1000, 1000}, File StrCat["res/For_Matlab/jx_planeOyz",Str_step,".pos"], Format Gmsh, OverrideTimeStepValue Time_step, LastTimeStepOnly];
					Print[ jy, OnPlane { {0,-Air_Ly/2,-Air_Lz/2} {0,-Air_Ly/2,Air_Lz/2} {0,Air_Ly/2,-Air_Lz/2} } {1000, 1000}, File StrCat["res/For_Matlab/jy_planeOyz",Str_step,".pos"], Format Gmsh, OverrideTimeStepValue Time_step, LastTimeStepOnly, SendToServer "No"];
					Print[ jz, OnPlane { {0,-Air_Ly/2,-Air_Lz/2} {0,-Air_Ly/2,Air_Lz/2} {0,Air_Ly/2,-Air_Lz/2} } {1000, 1000}, File StrCat["res/For_Matlab/jz_planeOyz",Str_step,".pos"], Format Gmsh, OverrideTimeStepValue Time_step, LastTimeStepOnly, SendToServer "No"];
					
					// Plane OXY
					Print[ jx, OnPlane { {-Air_Lx/2,-Air_Ly/2,0} {Air_Lx/2,-Air_Ly/2,0} {-Air_Lx/2,Air_Ly/2,0} } {1000, 1000}, File StrCat["res/For_Matlab/jx_planeOxy",Str_step,".pos"], Format Gmsh, OverrideTimeStepValue Time_step, LastTimeStepOnly, SendToServer "No"];
					Print[ jy, OnPlane { {-Air_Lx/2,-Air_Ly/2,0} {Air_Lx/2,-Air_Ly/2,0} {-Air_Lx/2,Air_Ly/2,0} } {1000, 1000}, File StrCat["res/For_Matlab/jy_planeOxy",Str_step,".pos"], Format Gmsh, OverrideTimeStepValue Time_step, LastTimeStepOnly, SendToServer "No"];
					Print[ jz, OnPlane { {-Air_Lx/2,-Air_Ly/2,0} {Air_Lx/2,-Air_Ly/2,0} {-Air_Lx/2,Air_Ly/2,0} } {1000, 1000}, File StrCat["res/For_Matlab/jz_planeOxy",Str_step,".pos"], Format Gmsh, OverrideTimeStepValue Time_step, LastTimeStepOnly, SendToServer "No"];												
					...
					EndIf					
				EndIf
				// B along the line 05 away and perpendicular line
				If(Num_Super == 4)
					Print [ b, OnLine {{0,DFromSurf,-Air_Lz/2}{0,DFromSurf,Air_Lz/2}} {500}, Format TimeTable, File StrCat["res/For_Matlab/Cut_B_alongx_3Bulks_05away",Str_step,".txt"] ];
					Print [ b, OnLine {{0,-Air_Ly/2,0}{0,Air_Ly/2,0}} {500}, Format TimeTable, File StrCat["res/For_Matlab/Cut_B_alongz",Str_step,".txt"] ];
				Else
					Print [ b, OnLine {{0,ay~{Stationnary_Sample}/2 + DFromSurf,-Air_Lz/2}{0,ay~{Stationnary_Sample}/2 + DFromSurf,Air_Lz/2}} {500}, Format TimeTable, File StrCat["res/For_Matlab/Cut_B_alongx_3Bulks_05away",Str_step,".txt"] ];
					Print [ b, OnLine {{0,-Air_Ly/2,0}{0,Air_Ly/2,0}} {500}, Format TimeTable, File StrCat["res/For_Matlab/Cut_B_alongz",Str_step,".txt"] ];
				EndIf
				// B on a line on the surface of the stationnary super
				Print [ b, OnLine {{0,0,(az~{Sample_1}/2)+0.0005}{2*ax_1218,0,(az~{Sample_1}/2)+0.0005}} {500}, Format TimeTable, File "res/Cut_B_alongx_05away.txt" ];
				Print [ b, OnLine {{0,0,(az~{Sample_1}/2)+0.0005}{0,2*ay_1218,(az~{Sample_1}/2)+0.0005}} {500}, Format TimeTable, File "res/Cut_B_alongy_05away.txt" ];
			EndIf		
			
			If((Active_approach == 0))				
				// Verif the current density
				Print[ jx, OnPlane { {0,-Air_Ly/2,-Air_Lz/2} {0,-Air_Ly/2,Air_Lz/2} {0,Air_Ly/2,-Air_Lz/2} } {1000, 1000}, File StrCat["res/For_Matlab/jx_planeOyz",Str_step,".pos"], Format Gmsh];
				Print[ jy, OnPlane { {0,-Air_Ly/2,-Air_Lz/2} {0,-Air_Ly/2,Air_Lz/2} {0,Air_Ly/2,-Air_Lz/2} } {1000, 1000}, File StrCat["res/For_Matlab/jy_planeOyz",Str_step,".pos"], Format Gmsh];
				Print[ jz, OnPlane { {0,-Air_Ly/2,-Air_Lz/2} {0,-Air_Ly/2,Air_Lz/2} {0,Air_Ly/2,-Air_Lz/2} } {1000, 1000}, File StrCat["res/For_Matlab/jz_planeOyz",Str_step,".pos"], Format Gmsh];
			EndIf
