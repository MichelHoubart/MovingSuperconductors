...
Resolution {
    ...
    { Name MagDyn;
        System {
            If(formulation == h_formulation)
				If(Flag_Mvt_hformulation == 0)
					{Name A; NameOfFormulation MagDyn_htot;}
				Else
					{Name A; NameOfFormulation MagDyn_htot_WithMVT;}
				EndIf
            ElseIf(formulation == a_formulation)
        ...
        Operation {
            ...
            SetTime[ timeStart + (Time_step - 1)*dt];
            SetDTime[ dt ];
            SetTimeStep[ (Time_step - 1) ];
            Evaluate[ $iter = 0 ];
			...
			Else
                ...
                // ...otherwise, reduce the time step and try again (should not go in here when Active_approach == 1)
                {
                    // Replace the rotor at its previous position
                    If(Flag_MB == 1 && Flag_rotating == 1)
                        ChangeOfCoordinates[ NodesOf[Rotor_Moving], RotatePZ[-delta_theta[]]];
				...
				// PostOperation[Projection_Visualisation];
			EndIf
