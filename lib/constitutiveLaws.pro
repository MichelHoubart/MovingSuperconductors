    ...
    // a-formulation: Power law j(e) = sigma(e) * e, with sigma(e) = jc/ec^(1/n) * |e|^((1-n)/n)
		If(Flag_JcB == 0)
			...
			djde[Super] = ($iter > -1) ? ((1.0/$relaxFactor) *
				( jc[] / ec * (1.0 / (epsSigma + ( (Norm[$1]/ec)#3 )^((n[]-1.0)/n[]) ))#4 *Anys_Matrix[]
				+ jc[]/ec^3 * (1.0-n[])/n[] * (#4)^(2) * 1/((#3)^((n[]+1.0)/n[]) + epsSigma2 ) * SquDyadicProduct[$1]*Anys_Matrix[]))
					: (jc[] / ec * 1.0 / ( epsSigma + ( Norm[$1]/ec )^((n[]-1.0)/n[]) ) * TensorDiag[1, 1, 1]*Anys_Matrix[] );
			//Isotorpic behaviour
			// djde[Super] = ($iter > -1) ? ((1.0/$relaxFactor) *
