    ...
    { Name MagDyn_coupled; Type FemEquation;
        ...
        Equation {
            ...
            // Time derivative - previous solution
             Galerkin { [ - (($TimeStep == 1 && (Flag_Test_projection == 1)) || ((Active_approach == 1)||(Active_approach == 2))) * mu[] * h_fromFile[]##2 / $DTime , {h} ];
                In MagnLinDomain; Integration Int; Jacobian Vol;  }
            Galerkin { [ - ((Active_approach == 0 && Flag_Test_projection == 0 || $TimeStep > 1)) * mu[] * {h}[1] / $DTime , {h} ];
                In MagnLinDomain; Integration Int; Jacobian Vol;  }
            // Induced currents
            // Non-linear OmegaC
            ...
            Galerkin { [ - (($TimeStep == 1 && (Flag_Test_projection == 1)) || ((Active_approach == 1)||(Active_approach == 2))) * a_fromFile[]##1 /\ Normal[] /$DTime , {h}];
                In BndOmega_ha; Integration Int; Jacobian Sur; }
            Galerkin { [ - ((Active_approach == 0 && (Flag_Test_projection == 0 || $TimeStep > 1))) * {a}[1] /\ Normal[] /$DTime , {h}];
                In BndOmega_ha; Integration Int; Jacobian Sur; }
